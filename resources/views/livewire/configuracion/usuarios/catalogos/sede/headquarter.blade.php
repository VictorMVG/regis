            ...
            <tbody>
                @foreach ($headquarters as $headquarter)
                    <tr wire:key="{{ $headquarter->id }}"
                        ...
                        <th scope="row"
                            class="px-4 py-3 font-medium text-gray-900 text-wrap dark:text-white">
                            {{ $headquarter->description }}
                        </th>
                        <th scope="row"
                            class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            {{ $headquarter->status->name }}
                        </th>
                        {{-- <td class="px-4 py-3 flex items-center justify-center relative" x-data="{ open: {}, up: false }">
                            <x-action-td :id="$headquarter->id" :routes="[
                                ['name' => 'Edit', 'route' => 'headquarters.edit'],
                                ['name' => 'Destroy', 'route' => 'headquarters.destroy', 'method' => 'DELETE'],
                            ]" />
                        </td> --}}
                        <td class="py-2">
                            <div class="flex flex-wrap justify-between items-center">
                                {{-- @haspermission('VER DETALLES DE LA SEDE')
                                    <a href="{{ route('headquarters.show', $headquarter) }}"
                                ...
                                @if (auth()->user()->hasRole(['SUPER USUARIO', 'ADMINISTRADOR GENERAL', 'ADMINISTRADOR DE SEDE']) ||
                                        $headquarter->user_id === auth()->id())
                                    @haspermission('EDITAR SEDE')
                                        <a href="{{ route('headquarters.edit', $headquarter) }}"
                                ...
                                @haspermission('ELIMINAR SEDE')
                                    <form id="delete-form-{{ $headquarter->id }}"
                                        action="{{ route('headquarters.destroy', $headquarter) }}" method="POST"
                                        class="flex justify-center items-center w-1/2" title="Eliminar">
                                        @csrf
                                        @method('DELETE')

                                        <button type="button" onclick="confirmDelete('delete-form-{{ $headquarter->id }}')"
