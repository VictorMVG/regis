<div class="bg-white dark:bg-gray-800 relative shadow-md rounded-lg overflow-hidden">
    ...
    <div class="overflow-x-auto pb-32">
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <x-head>
                <x-header>{{ __('User') }}</x-header>
                <x-header>{{ __('Email') }}</x-header>
                <x-header>{{ __('Rol') }}</x-header>
                <x-header>{{ __('Headquarter') }}</x-header>
                    ...
                    <tr wire:key="{{ $user->id }}"
                        ...
                        {{-- <td class="px-4 py-1 flex items-center justify-center relative" x-data="{ open: {}, up: false }">
                            <x-action-td :id="$user->id" :routes="[
                                ...
                                ['name' => 'Destroy', 'route' => 'users.destroy', 'method' => 'DELETE'],
                            ]" />
                        </td> --}}
                        <td class="py-2">
                            <div class="flex flex-wrap justify-between items-center">
                                {{-- @haspermission('VER DETALLES DEL USUARIO')
