<div class="bg-white dark:bg-gray-800 relative shadow-md rounded-lg overflow-hidden">
    ...
    <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 px-1 py-2">
        ...
        <div
            ...
            <!-- Add Buttons -->
            <a href="{{ route('observation-types.create') }}">
        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @foreach ($observationTypes as $observationType)
                    <tr wire:key="{{ $observationType->id }}"
                        ...
                        {{-- <td class="px-4 py-1 flex items-center justify-center relative" x-data="{ open: {}, up: false }">
                            <x-action-td :id="$observationType->id" :routes="[
                                ['name' => 'Edit', 'route' => 'observation-types.edit'],
                                ['name' => 'Destroy', 'route' => 'observation-types.destroy', 'method' => 'DELETE'],
                            ]" />
                        </td> --}}
                        <td class="py-2">
                            <div class="flex flex-wrap justify-between items-center">
                                ...
                                @if (auth()->user()->hasRole(['SUPER USUARIO', 'ADMINISTRADOR GENERAL', 'ADMINISTRADOR DE SEDE']) ||
                                        $observationType->user_id === auth()->id())
                                    @haspermission('EDITAR CATALOGO')
                                        <a href="{{ route('unit-types.edit', $observationType) }}"
                                ...
                                @haspermission('ELIMINAR CATALOGO')
                                    <form id="delete-form-{{ $observationType->id }}"
                                        action="{{ route('unit-types.destroy', $observationType) }}" method="POST"
                                        class="flex justify-center items-center w-1/2" title="Eliminar">
                                        @csrf
                                        @method('DELETE')

                                        <button type="button" onclick="confirmDelete('delete-form-{{ $observationType->id }}')"
